...
public class ComplexDouble extends AbstractField<ComplexDouble> {
	...
	public static final ComplexDouble MONE = new ComplexDouble(-1,0);
	...
	public ComplexDouble addInverse() {
		...
		DoubleField real = v.getValue(NNegInt.ZERO), imag = v.getValue(NNegInt.ONE);
	...
	public ComplexDouble add(ComplexDouble another) {
		...
		v1 = v1.add(v2);
		DoubleField val1 = v1.getValue(NNegInt.ZERO), val2 = v1.getValue(NNegInt.ONE);
	...
	public ComplexDouble constructOne(){return ONE;}
	...
	public GenericVSpace<DoubleField> getAsVector(){
		...
		if(angle.equals(ComplexSphere.NEUTRAL)){
			complexVec.setEntry(NNegInt.ZERO, radius);
			return complexVec;
		}
		if(angle.equals(ComplexSphere.HALF)){
			complexVec.setEntry(NNegInt.ZERO, DoubleField.M_ONE.multiply(radius));
			return complexVec;
		}
		if(angle.equals(ComplexSphere.QUARTER)){
			complexVec.setEntry(NNegInt.ONE, radius);
			return complexVec;
		}
		if(angle.equals(ComplexSphere.THREE_QUARTER)){
			complexVec.setEntry(NNegInt.ONE, DoubleField.M_ONE.multiply(radius));
			return complexVec;
		}
		double phi = 2*Math.PI*angle.getPhi();
		complexVec.setEntry(NNegInt.ZERO, radius.multiply(new DoubleField(Math.cos(phi))));
		complexVec.setEntry(NNegInt.ONE, radius.multiply(new DoubleField(Math.sin(phi))));
		return complexVec;
	}
