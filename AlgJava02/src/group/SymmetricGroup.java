...
import field.finitefield.TwoCycle;
import group.representation.PermutationRep;
import homomorphism.group.GroupHomo;
import homomorphism.group.InnerAuto;
 ...
 * a module of the symmetric group via the action described by each permutation on an
 * <tt>n</tt>-tuple
 */
public class SymmetricGroup<X extends Object> extends AbstractNAG<SymmetricGroup<X>>implements Function<Tuple<X>,Tuple<X>> {
	 ...
	 */
	public <F extends UnitaryCommRing<F>> AbstractRMatrix<F> constructPermutatMat (F one) {
		PermutationRep<F,X> permRep = new PermutationRep<F,X>(one);
		permRep.f(this);
		return permRep.getValue();
	}
	
	public <F extends UnitaryCommRing<F>> AbstractRMatrix<F> constructPermutationMat(F one, NNegInt rank){
		if(maxArg==0) setMaxArg();
		PermutationRep<F,X> permRep;
		if(maxArg<=(int) rank.eval()) 
			permRep = new PermutationRep<F,X> (one,rank);
		else permRep = new PermutationRep<F,X> (one,new NNegInt(maxArg));
		permRep.f(this);
		return permRep.getValue();
	}
	...
	public boolean equals(SymmetricGroup<X> another) {
		...
		if(cycleMap.size()!=another.cycleMap.size()) return false;
		if(cycleMap.size()==0&&another.cycleMap.size()==0) return true;
		return multiply(another.inverse()).isNeutral()?true:false;
	}
	public boolean equals(Object o){
		if(!(o instanceof SymmetricGroup)) return false;
		SymmetricGroup<Object> cp1 = new SymmetricGroup<Object>(), cp2 = new SymmetricGroup<Object>();
		cp1.cycleMap.addAll(cycleMap);
		cp2.cycleMap.addAll(((SymmetricGroup<?>) o).cycleMap);
	...
	public void f() {
		...
		for (Cycle cycle : cycleMap) {
			...
			while(it.hasNext()){
				...
				NNegInt index = new NNegInt(cycleEntry.getKey()), newIndex = new NNegInt(cycleEntry.getValue());
				X entry;
				if((entry = arg.getValue(index))!=null) val.setEntry(newIndex, entry);
				else val.remove(newIndex);
			}
		}
	}
	...
	/**
	 * Returns the conjugate permutation 
	 * <tt>another.inverve().multiply(this.multiply(another))</tt> of this
	 * @param another the conjugator
	 * @return the conjugate
	 */
	public SymmetricGroup<X> getConjugate(SymmetricGroup<X> another){
		return another.inverse().multiply(multiply(another));
	}
	/**
	...
	public int getMaxArg(){
		if(maxArg==0&&isNeutral()) return 0;
		if(maxArg==0) setMaxArg();
		return maxArg;}
	...
	public int hashCode(){
		int hash = 0;
		for (Cycle cycle:cycleMap) {
			hash += cycle.hashCode()*cycle.length;
		}
		return hash;
	}
	/**
	...
	public boolean isNeutral(){return cycleMap.size()==0?true:false;}
	/**
	...
	public SymmetricGroup<X> multiply (SymmetricGroup<X> another){
		...
		boolean thisSizeZero = cycleMap.size()==0, anothSizeZero = another.cycleMap.size()==0;
		if(thisSizeZero||anothSizeZero)
			return thisSizeZero&&anothSizeZero?new SymmetricGroup<X>():
				thisSizeZero?new SymmetricGroup<X> (another):new SymmetricGroup<X> (this);
		...
		while (true){
			...
			//last key came from another (right)
				...
				if(keys==null) keys = c2Right.getValue(key);
	 ...
	 * @param <X>
	 */
	public final static class Sign<X> extends AbstractFct<SymmetricGroup<X>,TwoCycle> implements GroupHomo<Sign<X>,SymmetricGroup<X>,TwoCycle> {
		/**
	...
	/**
	 * Returns the conjugation map for the permutation <tt>conjug</tt>
	 * @param conjug the permutation
	 * @return the conjugation map
	 */
	public static <X> InnerAuto<SymmetricGroup<X>> conjugation(SymmetricGroup<X> conjug){
		return new InnerAuto<SymmetricGroup<X>> (conjug);
	}
//	
//	public static <X> HashSet<SymmetricGroup<X>> permutationSet(Set<X> set){
//		//TODO fix method
//		HashSet<SymmetricGroup<X>> permSet = new HashSet<SymmetricGroup<X>>();
//		int length = set.size(), factorial = factorial(length);
//		permSet.add(new SymmetricGroup<X>());
//		for (int i = 1; i < length; i++){
//			SymmetricGroup<X> trans = new SymmetricGroup<X>(new int[]{0,i});
//			permSet.add(trans);
//			InnerAuto<SymmetricGroup<X>> innerTra = conjugation(trans);
//			for (int j = 2; j < length; j++){
//				int[] iCycle = new int[j+1];
//				for (int k = 0; k < j+1;k++) iCycle[k] = k;
//				SymmetricGroup<X> cycle = new SymmetricGroup<X>(iCycle);
//				innerTra.f(cycle);
//				permSet.add(cycle);
//				permSet.add(innerTra.getValue());
//			}
//		}
//		HashSet<SymmetricGroup<X>> perm2 = new HashSet<SymmetricGroup<X>> ();
//		while(permSet.size()<factorial){
//			for (SymmetricGroup<X> perms1 : permSet) {
//				if(perms1.isNeutral())continue;
//				InnerAuto<SymmetricGroup<X>> inner = conjugation(perms1);
//				for (SymmetricGroup<X> perms2:permSet){
//					if(perms1.equals(perms2) || perms2.isNeutral()) continue;
//					String str1 = perms1.toString(), str2 = perms2.toString();
//					System.out.println(String.format("(%1$s,  %2$s)",str1,str2));
//					if(str1.contains("(0,1)")&&str2.contains("(0,2,1,3)"))
//						System.out.println("Ouch");
//					inner.f(perms2);
//					SymmetricGroup<X> v = inner.getValue();
//					perm2.add(v);
//					perm2.add(v.inverse());
//				}
//			}
//			permSet.addAll(perm2);
//		}
//		return permSet;
//	}
	public static void main (String[] args){
		SymmetricGroup<Integer> sym1 = new SymmetricGroup<Integer>(
				new int[]{0,1,2}//,3,4}
				);
		SymmetricGroup<Integer> sym2 = new SymmetricGroup<Integer>(
				new int[]{2,4}//,5}//,7}
				);
		System.out.println(String.format("[%1$s, %2$s] = %3$s", sym1,sym2,sym2.commute(sym1.inverse())));
		/*SymmetricGroup<Integer> sym1 = new SymmetricGroup<Integer> (new int[]{0,1,0,2,4});
		SymmetricGroup<Integer> sym2 = new SymmetricGroup<Integer> (new int[]{0,1,3}), sym3 = sym1;
		InnerAuto<SymmetricGroup<Integer>> conj = conjugation(sym2);
		String out = "%1$s (%2$s) = \n%3$s";
		for (int i = 0; i < 3;i++){
			conj.f(sym3);
			System.out.println(String.format(out,conj,sym3,conj.getValue()));
			sym3 = conj.getValue();
		}*/
	}
	static final int factorial(final int val){
		int start = 1, count = 1;
		if(val<=0) return start;
		while (count<=val) {start=count*start;count++;}
		return start;
	}
