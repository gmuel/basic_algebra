...
public class SymmetricGroup2<X> extends AbstractNAG<SymmetricGroup2<X>> implements
	...
	public void f() {
		...
		for (Cycle cycle : cycleSet){
			for (Entry<Integer,Integer> cycleEntry:cycle.cycle.entrySet()){
				...
				NNegInt index = new NNegInt(argIndex);
				X entry = null;
				if((entry = arg.getValue(index))!=null) val.setEntry(new NNegInt(valIndex), entry);
			}
		}
		for (Entry<NNegInt,X> entry : arg){
	...
	public Cycle getCycle (NNegInt index){
		return getCycle((int) index.getValue().getValue());
	}
	...
	public boolean isNeutral(){return cycleSet.size()==0?true:false;}
	/**
