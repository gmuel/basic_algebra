...
public class MonoPoly<M extends CompMonoid<M>,R extends Ring<R>>  extends AbstractAbel<MonoPoly<M,R>> implements
		Ring<MonoPoly<M,R>>, Iterable<Entry<M,R>>{//, Module<MonoPoly<M,R>,R,M>{
	...
	/**
	 * Constructs a polynomial specified by <tt>another</tt>
	 *  with an additional comparator object <tt>comp</tt>
	 * @param another the original polynomial
	 * @param comp the comparator
	 */
	...
	public MonoPoly<M,R> add (MonoPoly<M,R> another){
		MonoPoly<M,R> sum = new MonoPoly<M,R> (this);
	...
	public boolean equals (MonoPoly<M,R> another){
		Iterator<Entry<M,R>> it1 = iterator(), it2 = another.iterator();
		while (it1.hasNext()&&it2.hasNext()){
			Entry<M,R> entry1 = it1.next(), entry2 = it2.next();
			if(entry1.getKey().compareTo(entry2.getKey())!=0) return false;
			if(!entry1.getValue().equals(entry2.getValue())) return false;
		}
	...
	/**
	 * Returns a shallow copy of the coefficient
	 * map - <b>note</b> that changes to the returned
	 * map will be reflected in this polynomial
	 * @return the coefficient map
	 */
	protected TreeMap<M,R> getCoefficientMap(){return coeffMap;}
	/**
	...
	/**
	 * Returns a <code>TreeSet</code> of all
	 * indices of type <code>M</code> in
	 * this polynomial - to specify: all indices
	 * with non-zero coefficients or its so called
	 * support.
	 * <p><b>Note</b>, the returned set is a deep
	 * copy - changes to the set will not be reflected
	 * in this polynomial
	 * @return the support
	 */
	public TreeSet<M> getSupport (){
		return new TreeSet<M> (coeffMap.keySet());
	}
	public int hashCode (){
		int hash = 0;
	...
	public boolean isCommutative (){
		...
		for (Entry<M,R> entry:this){
			if(entry.getValue().isCommutative()) return true;
		}
		return false;
	}
	public boolean isDiscrete(){return getCoefficient(getDegree()).isDiscrete();}
	/**
	 * Returns true if this object
	 * is a constant polynomial
	 * with unit entry.
	 * <p><b>Note</b>, for a different
	 * behavior, simply subclass and
	 * override this method
	 */
	public boolean isUnit(){
		if(isZero()) return false;
		M deg = getDegree();
		if(!deg.isNeutral()) return false;
		R coeff = getCoefficient(getDegree());
		return coeff.isUnit();
	}
	...
	public MonoPoly<M,R> multiply(R scalar){
		MonoPoly<M,R> sclProd = new MonoPoly<M,R>();
		for (Entry<M,R> entry:this){
			sclProd.setCoefficient(entry.getKey(), entry.getValue().multiply(scalar));
		}
		return sclProd;
	}
	/**
