/*
 * utils_d_link_exp_c_ref.hpp
 *
 *  Created on: 01.12.2019
 *      Author: lmmac
 */

#ifndef INCLUDE_ALG_1_1_UTILS_UTILS_D_LINK_EXP_C_REF_HPP_
#define INCLUDE_ALG_1_1_UTILS_UTILS_D_LINK_EXP_C_REF_HPP_
...
//TODO: 'util::d_linked_exp_list<const X& >::iterator'
//TODO: and remove all other mutators ('insert', 'remove',...) or redirect to iterators
//TODO:
//TODO: is concurrency check essential? Should be left to client
/**
 * @brief Partial specialization of d_link_exp
 *
 * Usage only for compile-time constant instances of X, or instances of X within
 * a list's scope.
 * Furthermore, the type X has to provide a static public member
 * <cdn>const X& def_val()</cdn> returning some default value for its type
 * @tparam type, with static member fct 'def_val()'
 */
template<class X >
class d_link_exp<const X& > {

#ifdef D_LINK_EXP_C_REF_HPP_NO_MUTAT
	/**
	 * Addition member
	 * Appends all entries of entry_list
	 * to this
	 * @param entry_list
	 * @return a pointer to the head entry, i.e. with no
	 * 		predecessors
     * TODO should be left to iterator class in d_linked_exp_list
	 */
	d_link_exp<const X& >* add(const d_link_exp<const X& >& entry_list){
		const d_link_exp<const X& >* ptr1 = &entry_list;
		d_link_exp<const X& >* ptr2 = 0;
		...
		while(ptr1->prev) ptr1 = ptr1->prev;
		return const_cast<d_link_exp<const X& >* >(ptr1);
	}
	/**
	 * Addition member
	 ...
	 */
	d_link_exp<const X& >* add(const X& x, u_int indx){
		if(!ptr) {
			ptr = &x;
		...
		if(index<indx) {
			...
			if(next&&next->index>=indx){
				...
				d_link_exp<const X& >* l_ptr = new d_link_exp<const X& >;
				l_ptr->ptr = &x;
				l_ptr->index = indx;
				l_ptr->next = next;
				next->prev = l_ptr;
...
#endif /* D_LINK_EXP_C_REF_HPP_NO_MUTAT */
...
protected:
	 ...
	 */
	d_link_exp(const d_link_exp<const X& >& o):
		ptr(0),prev(0),
		next(0),length(0),
		index(o.index)
	{
	 ...
	 * @param list to copy
	 */
	d_link_exp(const d_linked_exp_list<const X& >& list):
		ptr(0),prev(0),
		next(0),length(0),
		index(0)
	{
	...
	//used to init d_link_exp<const X& >::DEFAULT_VAL
	//static const X& init_DEF();
	friend class d_linked_exp_list<const X& >;
...
public:
	 ...
	 */
	d_link_exp<const X& >& operator=(const X& x){
		ptr = &x;
		return *this;
	}
	/**
	 ...
	 * @return the altered list entry
	 * TODO should be left to iters or deprecated entirely
	 */
	d_link_exp<const X& >& operator=(const d_link_exp<const X& >& link){
		/*
		const d_link_exp<const X& >* ptr2 = &link;
		while(ptr1->prev) ptr1 = ptr1->prev;
		...
		if(ptr1&&!ptr2){
			...
			while(ptr2){
				ptr1->next = new d_link_exp<const X& >;
			...
			if(index>link.index){
				if(prev){
					if(prev->index<link.index){
						d_link_exp<const X& >* ptr1 = new d_link_exp<const X& >;
						ptr1->prev = prev;
						ptr1->next = this;
						prev->next = ptr1;
						prev = ptr1;
	 ...
	 */
	friend bool operator==(const d_link_exp<const X& >& i1, const d_link_exp<const X& >& i2){
		return i1.index==i2.index&&*i1.ptr==*i2.ptr;
	}
	/**
	 * @brief Inequality member
	 * @param i1 first link
	 * @param i2D_LINK_LIST_H01_UNSIZED_LINK second link
 ...
 * @tparam X the type of the list entries, <b>HAS TO</b> provide
 * 		static default member fct 'def_val()' or linkage error
 */
template<class X >
class d_linked_exp_list<const X& > {
public:
	class const_iterator;
	class iterator;
protected:
	typedef d_linked_exp_list<const X& > _list;
	/**An array view of the list*/
	d_link_exp<const X& >** entry_array;
	/**The list's head entry pointer*/
	d_link_exp<const X& >* first,
	...
	void init_array(){
		if(!size) return;
		if(entry_array) return;
		entry_array = new d_link_exp<const X& >*[size];
		for(u_int i = 0; i < size; i++) entry_array[i] = 0;
	}
	/**
	 * Auxiliary member
	...
	void insert(u_int index, const X& x){
		...
		if(entry_array){
			...
			d_link_exp<const X& >* ptr = entry_array[index],* ptr0 = 0;
			if(ptr){
				*ptr = x;
			}
			else{
		...
		if(first) {
			d_link_exp<const X& >* ptr = first->add(x,index);
			if(ptr==first) {
				last = ptr;
				del_array();
				size = last->index+1;
			...
			first = new d_link_exp<const X& >(x,index);
			last = first;
			size = new u_int(first->index+1);
			init_array();
		}
		set_array();
#else /*_D_LINK_LIST_H01_NO_ITER_INSERT*/
		d_linked_exp_list<const X& >* ths =const_cast<d_linked_exp_list<const X& >* >(this);
...
#endif /*_D_LINK_LIST_H01_NO_ITER_INSERT*/
	}
	/**
	 * Auxiliary member
	...
	bool remove(u_int index){
		if(entry_array){
			if(index<size){
				d_link_exp<const X& >* ptr = entry_array[index];
				if(ptr) {
					if(ptr==last) {
						if(last->prev){
							last->prev->next = 0;
	 ...
	 */
	void set_list(const d_link_exp<const X& >& list){
		...
		first = new d_link_exp<const X& >(*list,list.index);
		entry_array[first->index] = first;
		d_link_exp<const X& >* ptr0 = list.next,* ptr1 = first;
		while(ptr0) {
	...
	explicit d_linked_exp_list(INPUT_ITER& i, const INPUT_ITER& e):
		...
		if(i!=e){
			...
			while(i!=e){
				...
				const d_link_exp<const X& >* lnk = i.operator->();
				if(lnk){
...
public:
	 ...
	 * @param o original
	 */
	d_linked_exp_list(const d_linked_exp_list<const X& >& o):
			entry_array(0),
			first(0),last(0),
	 ...
	 */
	static const d_link_exp<const X& >& EMPTY_LIST;
	/**
	...
	class const_iterator {//: public std::iterator<std::random_access_iterator_tag,X >{
		 ...
		 * instance*/
		const d_linked_exp_list<const X& >* list;
		...
		* child;
		 ...
		 */
		const_iterator(const d_linked_exp_list<const X& >* list_ptr, bool rev_flag = false,
				bool is_end = false):
			list(list_ptr),
			parent(0),child(0),
	...
	public:
		 ...
		 * 		list entry in the list or <code>d_linked_exp_list<const X& >::EMPTY_LIST</code>
		 * 		if no more entries or end iterator
		 */
		const d_link_exp<const X& >& operator*() const {return ptr?*ptr:EMPTY_LIST;}
		/**
		 * @brief Pointer operator
		 * @return a pointer to <code>const d_link_exp&ltX&gt</code>
		 */
		const d_link_exp<const X& >* operator->() const {return ptr;}
		/**
		 * @brief Pointer operator
		 * @return a pointer to <code>d_link_exp&ltX&gt</code>
		 */
		d_link_exp<const X& >* operator->() {return ptr;}
		/**
	...
	class iterator {//: public std::iterator<std::forward_iterator_tag,X> {
		d_linked_exp_list<const X& >* list;
		...
		/**End iterator flag*/
		end_it;
		iterator(d_linked_exp_list<const X& >& lst, bool revFlag = false, bool flag = false):
	...
	public:
		...
		void insert(const X& x, u_int idx_hint = -1u, bool suppress_array = false){
			...
			if(ptr){
				...
				if(ptr->index==idx_hint) ptr->ptr = &x;
					...
					if(ptr->index>idx_hint) {
						if(ptr->prev) {ptr = ptr->prev;insert(x,idx_hint,true);}
							...
							ptr->prev = new d_link_exp<const X& >;
							ptr->prev->ptr = &x;
							ptr->prev->next = ptr;
							list->first = ptr->prev;
						...
						if(ptr->next){
							if(ptr->next->idx()<=idx_hint) {
								...
								d_link_exp<const X& >* ptr0 = new d_link_exp<const X& >;
								ptr0->ptr = &x;
							...
							ptr->next =  new d_link_exp<const X& >;
							ptr->next->ptr = &x;
							if(idx_hint) {
				...
				if(!list->first){
					list->first = new d_link_exp<const X& >(x,idx_hint);
		...
		throw(concurr_except){
			...
			if(list->has_const_iterator||list->has_muta_iterator){
				const d_link_exp<const X& >* lnk = const_cast<const d_link_exp<const X& >* >(ptr);
			...
			if(ptr) {
				d_link_exp<const X& >* ptr0 = ptr->prev,* ptr1 = ptr->next;
				if(ptr0&&ptr1){
					ptr0->next = ptr1;
					ptr1->prev = ptr0;
					ptr->prev = 0;
		 ...
		 * 	or NULL if terminal
		 */
		const d_link_exp<const X& >* operator->() const {
			return ptr?const_cast<const d_link_exp<const X& >* >(ptr):0;
		}
		/**
	 ...
	 */
	d_linked_exp_list<const X& >& operator=(const d_linked_exp_list<const X& >& o){
		if(this==&o) return *this;
		if(o.size&&size){
			if(size<o.size) {
				del_array();
	 ...
	 */
	const d_link_exp<const X& >& operator*() const {return first?*first:EMPTY_LIST;}
	/**
	 ...
	 */
	const d_link_exp<const X& >* operator[](u_int idx) const {
		return first?
				idx<*first->length?
						entry_array[idx]:0:
						0;
	 ...
	 */
	d_linked_exp_list<const X& > reverse() const {
		if(size==0||first==0) return *this;
		d_linked_exp_list<const X& > rev;
		rev.size = size;
	...
	friend class d_link_exp<const X& >;
};
...
#endif /* INCLUDE_ALG_1_1_UTILS_UTILS_D_LINK_EXP_C_REF_HPP_ */
